...
En el script ‘Ids Balanzas.sql’ se determinan los ids de los documentos a partir de las vistas que corresponden a balanzas de los meses 12 o 13, según corresponda, junto a su RFC (para poderlo procesar después en ‘paralelo’) y se escribe en una tabla local llamada ‘ids’.

  _Código de ‘Ids Balanzas.sql’_
```{sql, eval=FALSE}
...
De manera paralela al proceso anterior se puede ejecutar el script ‘Ids Catalogos.sql’ el cual de manera análoga al proceso contenido en ‘Ids Balanzas.sql’ revisa en todas las vistas de ambas bases de datos, extrae todos los catálogos aceptados y retiene el último (por contribuyente) hasta la fecha de interés escribiendo en una tabla local llamada ‘idsCatalogo’ los ids de los documentos que corresponden a los últimos catálogos, junto con otros campos como ‘RFC, ‘Mes’, ‘Anio’ y FechaRecepcion’. Esta tabla ‘idsCatalogo’ además de contener los ids a procesar posteriormente en paralelo ya contiene la información suficiente para identificar a los catálogos en las tablas de ‘Catalogo’ en ambas bases de datos por lo que se emplea en el scipt ‘split.sql’ para complementar la información contenida en las tablas ‘CtasCatalogo’ construir los catálogos de cada contribuyente.  
...
En este punto hay que destacar que ‘la balanza’ se integra por el join de las tablas ‘Balanza’ con ‘Ctas’ y a su vez ‘el catalogo’ se integra por el join de las tablas ‘Catalogo’ con ‘CtasCatalogo’. Después del join entre la tabla ‘Balanza’ y ‘Ctas’, ‘la balanza’ solo tiene números de cuenta (el campo ‘NumCta’) pero con saldos iniciales, finales, haberes y deberes; por su parte, después del join entre la tabla ‘Catalogo’ y ‘CtasCatalogo’, ‘el catalogo’ posee números de cuenta ‘NumCta’ y códigos de agrupación ‘CodAgrup’  así que como dice el tercer párrafo de la sección I de la regla [2.8.1.6](http://www.sat.gob.mx/informacion_fiscal/normatividad/Paginas/resolucion_miscelanea_2016.aspx) de la Resolución Miscelánea Fiscal para 2016 basta con hacer un match entre los números de cuenta de  ‘la balanza’ y ‘el catalogo’ (considerando que los números de cuenta varían de contribuyente en contribuyente). 

Con base en las tablas ’ids’ y ‘idsCatalogo’, que ya se tienen de manera local y en vista de que contienen información suficiente para identificar tanto a los catálogos y a las balanzas; en las tablas ‘Catalogos’, ‘CtasCatalogos’, ‘Balanza’ y ‘Ctas’, el script ‘split.sql’ comienzan dividiendo dichas tablas, cada una en 26 tablas en función de la letra con la que comienza el RFC asociado a cada registro. Las tablas se dividen para poder filtrar y copiar las tablas ‘Catalogos’, ‘CtasCatalogos’, ‘Balanza’ y ‘Ctas’ que viven en ambas bases de datos [DocumentoContabilidad-1] y [DocumentoContabilidad-2]. Después de filtrar y copiar las tablas ‘Catalogos’, ‘CtasCatalogos’, ‘Balanza’ y ‘Ctas’ para cada letra, se realizan los joins correspondientes para obtener las tablas ‘final_*’  que contiene los números de cuenta, códigos de agrupación y saldos por contribuyente. 


Las tablas 'final_agrupado_\*' contienen agregados de las tablas ‘final_\*’, los cuales finalmente se pueden concadenar y formar una única tabla. 
